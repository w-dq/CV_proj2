...
for dirpath, dirnames, fnames in subfolders:
	...
	for f in fnames:
		try:
			total.extend(utils.get_repr(dirpath+'/'+f))
		except:
			continue
	with open('centers512/{}.json'.format(dirpath.strip(' ').split('/')[-1]),'w',encoding='utf-8') as file:
		print('dump:',dirpath)
		json.dump(utils.get_model(total,512).tolist(),file)
	i+=1
